      ...
      <CardContent className="space-y-3">
        {signals.map((conflict, i) => (
          <div key={i} className="rounded-lg border border-border/30 bg-muted/10 p-3 space-y-2">
            <Badge variant="secondary" className={`text-[10px] ${categoryColors[conflict.category] || "bg-muted text-muted-foreground border-border/50"}`}>
              {conflict.category}
            </Badge>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
              <div className="space-y-1">
                <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Source A</p>
                <p className="text-sm text-foreground">{conflict.signalA}</p>
                <p className="text-[11px] text-muted-foreground">{conflict.sourceA}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Source B</p>
