...
SELECT cron.unschedule('run-monitored-searches-hourly')
WHERE EXISTS (
  SELECT 1
  FROM cron.job
  WHERE jobname = 'run-monitored-searches-hourly'
);

SELECT cron.schedule(
  'run-monitored-searches-hourly',
  '*/15 * * * *',
  $$
  SELECT net.http_post(
    url := 'https://vnileremxagzmlieykgm.supabase.co/functions/v1/run-monitored-searches',
